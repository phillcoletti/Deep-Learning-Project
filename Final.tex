...
We also tested randomly selecting the type of corruption at each epoch of training. Models trained using this method approximated, but unfortunately never surpassed those using regular dropout. Their average error was 1.12\%. A plot of their training and test error during training can be seen below (Figure \ref{fig:mixedError}). Note that no overfitting occurs because the learning rate steadily decays during training.
...
\begin{minipage}{.75\textwidth}
  ...
  \subcaption{Various corrpution rates for regular dropout}
  \label{fig:ecr_drop}
  \includegraphics[width=.85\textwidth]{ensemble_rand}
  \subcaption{Various corrpution rates for random corruption}
  \label{fig:ecr_rand}
  \includegraphics[width=.85\textwidth]{ensemble_sp}
  \subcaption{Various corruption rates for salt \& pepper corruption}
  \label{fig:ecr_sp}
\end{minipage}
...
The quantitative results of the experiment for random and salt \& pepper noise (Table \ref{tab:PollingMethods1}) were not clear, because some of the corruption rates had only one or two models in the ensemble. It is not fair to compare the performance of these different voting schemes with so few models, becuase the lack of ensemble size hides any strengths or weaknesses of the respective prediction methods. Therefore we only analyze the results of the ensemble prediction methods for standard dropout corruption.

Mean polling and product polling perform very similarly at all corruption rates (Figure \ref{fig:PollingMethods2}). Product polling is a bit more sensitive to outliers as their effect is felt more through the multiplication operation than simple addition. The performance of these two methods tends to be in the middle of the pack at all corruption rates. In contrast, max, min, and median polling tend to find themselves closer to the extremes in performance. This makes sense, as they only use the output of one network for prediction, although all networks are considered when selecting the min, max, or median. Although competitive in performance (Figure \ref{fig:PollingMethods1}), median polling takes substantially longer than the other variations, as the network predictions must first be sorted. Because fast inference is extremely important, we did not include median polling in the second run of experiments with larger ensemble sizes (Figure \ref{fig:PollingMethods2}).
...
\begin{figure}[!htb] 
  \begin{minipage}[b]{0.5\linewidth}
    ...
    \subcaption{0.4 Corruption Rate} 
    \vspace{2ex}
  \end{minipage}%%
  \begin{minipage}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=\linewidth]{methods7} 
    \subcaption{0.5 Corruption Rate} 
    \vspace{2ex}
  \end{minipage} 
  \begin{minipage}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=\linewidth]{methods6} 
    \subcaption{0.6 Corruption Rate} 
  \end{minipage}%% 
  \begin{minipage}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=\linewidth]{methods5} 
    \subcaption{0.7 Corruption Rate} 
  \end{minipage} 
...
Our results clearly indicate that all forms of corruption, aside from Gaussian noise, improve model performance. Of the types of corruption we tested, traditional dropout, also known as zero-masking, worked the best. The improved performance of salt \& pepper noise in stacked denoising autoencoders (Vincent et al (2010)) did not carry over into supervised training.

Our results also clearly indicate that the corruption rate of 0.5 that is ideal for individual model performance (Hinton et al (2010)) also performs best in the ensemble learning setting. The additional stochasticity injected by corrupting more hidden units decreases individual model performance far more than any benefit it creates in terms of increase variance.

Our experiments also did not yield any conclusive results with regards to ensemble prediction methods. The optimal choice of polling method seems to depend on the context. It does, however, appear clear that performance did did not plateau even with 11 models in the ensemble. Thus, the learned networks had enough variance in their predictions that ensemble learning would have continued to increase perforamance with even larger ensemble sizes.
...
As ensemble learning appears to be so effective for these neural networks, future work should investigate other methods of increasing model variance. Using different parameter initialization schemes or varying the corruption rate by layer might yield more model variance without sacrificing individual model error rate. As hardware continues to improve, storing and polling large ensembles of neural networks may become feasible even on a commercial scale, making this area of research extremely valuable.
